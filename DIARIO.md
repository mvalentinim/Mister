...
## Sessione 35 — 2026-08-22 — M11: curve di crescita personali e il grafico della carriera

**Obiettivo dichiarato:** le curve di crescita/declino personalizzate dalla personalità (l'attività in lista da sessione 24), col dettaglio nuovo dello sviluppatore: un GRAFICO età/valore per ogni giocatore — per i miei si vede la curva ipotetica "come da DB" E la curva reale (che può fare meglio o peggio, in base a utilizzo e risultati); per i giocatori non miei solo la curva reale fino a oggi, e il POTENZIALE resta nascosto finché non li compro (nell'Editor del DB, fuori dalla carriera, si vede sempre tutto).

**Fatto:**
- **La curva personale nel motore** (`crescita.ts`): `parametriCurva(g)` deriva (deterministicamente) dalla personalità: la PROFESSIONALITÀ sposta l'età del picco (26-30 invece del fisso 28) e frena o aggrava il declino; l'AMBIZIONE spinge o smorza la crescita giovanile. Le fasce d'età di fine stagione ora scorrono col picco personale. Vale per tutti, rigenerati compresi (stessa personalità → stessa curva, e il potenziale resta quello del DB).
- **Crescita PROPORZIONALE al margine** — bug storico scovato dal collaudo del modello: coi passi fissi (+1..+3) un talento di 18 anni a 60 con potenziale 88 si fermava a 69! Ora chi è lontano dal suo tetto brucia le tappe (fase giovanile ~35% del gap l'anno, tetto +6; rifinitura ~20%; maturità ultimo 10%): il talento medio arriva a ridosso del potenziale (85 su 88 di attesa, il resto lo fanno dadi e utilizzo) — e un rigenerato sedicenne può davvero ridiventare campione, come da requisito.
- **Lo storico delle medie** (`storiaMedie`, salvataggio v14): fotografie [età, media] dei giocatori della MIA rosa alla creazione della carriera, a ogni fine stagione e alla firma di acquisti/svincolati (il primo punto della loro storia è la firma). Migrazione v13→v14 per i salvataggi vecchi (partono vuoti).
- **`CurvaCrescita.tsx`**: il grafico SVG in stile Almanacco — assi a china, griglia leggera, etichette monospazio; curva PREVISTA tratteggiata (dal primo punto certo, integrando i passi attesi del motore fino a 40 anni), curva REALE verde piena fino all'età di oggi (storico + passato ricostruito a ritroso), filetto oro del potenziale con etichetta POT. Per i non-miei: solo la reale, con la dicitura "potenziale e previsione: riservati".
- **La scheda giocatore IN CARRIERA** (`SchedaGiocatoreCarriera.tsx`): finestra sovrapposta con figurina + curva. Dalla Rosa (che prima non apriva nulla!) per i miei; dal Mercato (nomi cliccabili in ricerca, svincolati e contratti) per tutti. La figurina ora sa nascondere il potenziale (POT ??) e mostrare l'età di stagione (in carriera gli anni avanzano anche se l'orologio no).
- **`npm run curve`**: nuovo test del modello (9 verifiche: picco spostato dalla professionalità, spinta dell'ambizione, declino frenato/aggravato, curva reale che passa per lo storico e finisce a oggi, ricostruzione del passato).
...
**Appendice sessione 36:** creato `docs/collaudo-di-persona.md` — la lista completa del collaudo di persona (DoD M0-M11) per il ritorno del Mac: 15 sezioni nell'ordine di una partita vera (preparazione e test automatici, titolo, database, editor+volti, wizard carriera, HUD, mercato con la regola del potenziale nascosto, tattica, Match Day con tutti i suoni, verdetti di stagione, curve di crescita, carriera lunga con rigenerati, portabilità .mister cross-device, multi-nazione a mano, giudizio finale §7), con caselle da spuntare e spazio per gli appunti. Alla prossima sessione si leggono le annotazioni e si sistemano le stonature.
